...
def mostrar_tela(user_nome: str, perfil_global: str = ""):
    ...
    try:
        ...
        notificar("Erro ao exibir a tela Técnico", type="negative")
    ...
    try:
        ...
        def render():
            try:
                ...
                with wrap:
                    ...
                    def baixar():
                        try:
                            ...
                            try:
                                try:
                                    _log().exception("baixar falhou")
                                except NameError:
                                    try:
                                        log.exception("baixar falhou")
                                    except NameError:
                                        from mod_intranet import observabilidade as _obs_fail
                                        _obs_fail.get_logger("tecnico").exception("baixar falhou")
                            except Exception:
                                pass
                            try:
                ...
                log.exception(f"render software falhou: {e}")
                notificar("Erro ao listar software", type="negative")

        render()
    except Exception:
        try:
            try:
                _log().exception("_painel_software falhou")
            except NameError:
                try:
                    log.exception("_painel_software falhou")
                except NameError:
                    from mod_intranet import observabilidade as _obs_fail
                    _obs_fail.get_logger("tecnico").exception("_painel_software falhou")
        except Exception:
            pass
        try:
            from mod_intranet.tema_modulo import notificar as _notificar_fail
                ...
                with wrap:
                    ...
                    with ui.card().classes("w-full p-4 gap-3"):
                        ...
                        def criar():
                            try:
                                ...
                                log.exception(f"criar pasta backup falhou: {e}")
                                notificar(f"Falha ao criar pasta: {e}", type="negative")
                    ...
                    async def ao_upload(e):
                        try:
                            ...
                            log.exception(f"ao_upload falhou: {ex}")
                            notificar(f"Falha no envio: {ex}", type="negative")
                                    ...
                                    with ui.row().classes("gap-2"):
                                        def _baixar(p=pasta_nome):
                                            try:
                                                ...
                                                log.exception(f"baixar backup falhou {p}: {ex}")
                                        ...
                                        def _listar(p=pasta_nome):
                                            try:
                                                ...
                                                log.exception(f"listar backup falhou {p}: {ex}")
                                                notificar(f"Erro ao listar arquivos: {ex}", type="negative")
                                        botao_icone("visibility", on_click=_listar, tooltip="Ver arquivos", chave_modulo="tecnico")

                    with ui.row().classes("w-full justify-center mt-2"):
                        botao("Atualizar", icone="refresh", on_click=render, variante="texto", chave_modulo="tecnico")
            except Exception as e:
                log.exception(f"render backup falhou: {e}")
                notificar("Erro ao exibir painel de backup", type="negative")

        render()
    except Exception:
        try:
            try:
                _log().exception("_painel_backup falhou")
            except NameError:
                try:
                    log.exception("_painel_backup falhou")
                except NameError:
                    from mod_intranet import observabilidade as _obs_fail
                    _obs_fail.get_logger("tecnico").exception("_painel_backup falhou")
        except Exception:
            pass
        try:
            from mod_intranet.tema_modulo import notificar as _notificar_fail
...
def _dlg_listar(pasta_nome: str, user_nome: str):
    try:
        ...
        try:
            ...
            log.exception(f"_dlg_listar falhou: {pasta_nome}")
            notificar(f"Erro ao listar arquivos: {ex}", type="negative")
    except Exception:
        try:
            try:
                _log().exception("_dlg_listar falhou")
            except NameError:
                try:
                    log.exception("_dlg_listar falhou")
                except NameError:
                    from mod_intranet import observabilidade as _obs_fail
                    _obs_fail.get_logger("tecnico").exception("_dlg_listar falhou")
        except Exception:
            pass
        try:
            from mod_intranet.tema_modulo import notificar as _notificar_fail
