...
| `mod_agregador_noticias/` | `db_mod_agregador_noticias.db` | ✓ | **novo** (19/09/2026) — multi-fonte `httpx+parsel` (Google/BBC/JFP/RSS) grid responsivo **3→2→1 + altura 160–220px** `ui.link(target=href, new_tab=True)` + **busca NFKD lado a lado filtro tema** `agregador-busca` `debounce 300` `placeholder "Buscar palavra…"` `clearable` (`agregador-filtro-tema` + `agregador-busca` lado a lado, **sem badge `Coleta:`** removido a169c9a), 24h `tb_noticia` (`fonte_icon_url` 16×16 `faviconV2` + `imagem_url` 30×30 `/api/attachments`), dedup `_norm_tit` + `COALESCE` + `fonte_icon`/`imagem` extração, grid responsivo **`.grid-noticias {display:grid; grid-template-columns: repeat(3,1fr)}` + `@media 1024px→2 640px→1` + `.card-noticia {min-height:160px; max-height:220px}` (média 91 notícias: título 79/75, descrição 34, 87.9% imagem, 24% fonte_icon)** + paginação 12 + busca 500 NFKD, **tela pura** `/agregador-noticias-puro` (`telas_puro.py:mostrar_tela_pura` `intro-overlay` + `about` `info-list` com cards atuais 3 cols `fonte_icon 16×16` + `imagem 30×30`, `assets/noticia/` `base.css`/`vendor.css`/`main.css`/`font-awesome`/`micons`/`fonts/lora/poppins`/`images/bg/intro`/`js/modernizr/pace/jquery/plugins/main.js` via `main.py:818` `@app.get("/assets/noticia/{caminho:path}")` + botão `Ver puro Noticia` `data-testid=agregador-ver-puro`), `listar_para_tv` (`fonte_icon` incluso, censura `conteudo_palavras_bloqueadas` + `limpar_censuradas`), termo+fontes configuráveis |
...
| `mod_agregador_noticias/` | `mod_agregador_noticias/telas_puro.py` + `assets/noticia/` (tema Noticia 4.9M `css→base.css/vendor.css/main.css/font-awesome/micons, fonts→lora/poppins, images→bg.jpg/intro-bg.jpg, js→jquery-2.1.3/modernizr/pace/plugins/main`, `favicon.png` via `main.py:818` `@app.get("/assets/noticia/{caminho:path}")` path-traversal safe) — tela pura `/agregador-noticias-puro` (`intro-overlay` + `about` `info-list` com cards atuais 3 cols `fonte_icon 16×16` + `imagem 30×30`, 12 notícias `listar_noticias(limite=12)`) + `mod_filas/midia/` (`PASTA_MIDIA` `/midia_filas` 12K) — banco `tb_noticia` (`fonte_icon_url` 16×16 + `imagem_url` 30×30) grid responsivo **`.grid-noticias {display:grid; grid-template-columns: repeat(3,1fr)}` + `@media 1024px→2 640px→1` + `.card-noticia {min-height:160px; max-height:220px}` (média 91 notícias: título 79/75, descrição 34, 87.9% imagem, 24% fonte_icon)** paginado 12 + **busca** (`por_pagina=12` `COALESCE DESC` `Primeira/Anterior/Próxima/Última` `agregador-primeira/anterior/proxima/ultima` + `agregador-busca` `debounce 300` NFKD `placeholder "Buscar palavra…"` lado a lado filtro tema, **sem badge `Coleta:`**), **ícone fonte 16×16 + imagem 30×30 lado a lado antes do título + botão `Ver puro Noticia` `agregador-ver-puro` → `/agregador-noticias-puro`**, sem backup — banco reciclado `reiniciar_banco` `06:00` `CronTrigger` + coleta `httpx+parsel` + `listar_para_tv` (`fonte_icon` incluso) + `assets/noticia/` servido via rota estática 4.9M |
