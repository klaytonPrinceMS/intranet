...
| **`pyflakes` (`>=3.2.0`)** | **análise estática de código Python — `undefined name` é BLOQUEANTE antes de fechar o ciclo de testes** (seção "Lint / análise estática" do `requirements-dev.txt`; **nunca** em `requirements.txt`) | ✅ Instalado (dev) |
| `GEMINI_API_KEY` (opcional) | passada semântica de `docs/`; sem ela, o agente host assume | ⚠️ Opcional |

!!! danger "`pyflakes` não é opcional porque o `try/except` esconde `NameError`"
    O padrão obrigatório do AGENTS.md §3.2 (envolver toda função em `try/except`
    para nunca derrubar o servidor) **engole o `NameError`** quando o `except`
    termina em `pass`. Resultado: a suíte passa, o terminal fica limpo e a
    funcionalidade está quebrada. Em 25/09/2026 foram **7 `undefined name` em 5
    arquivos** com efeito real e nenhum sinal. Comando:
    ...
    Detalhe em [Convenções — Análise estática obrigatória](convencoes_codigo.md#analise-estatica-obrigatoria-pyflakes).
...
| Requisito | Situação |
|:---|:---:|
| Banco `db_mod_agregador_noticias.db` (`tb_noticia` 11 colunas, `data_publicacao` real via `_parse_data_pub`, `data_coleta`, índices `tema/fonte/data`) + seeds `tb_config` (`habilitado 0`, `intervalo 60`, `temas_json`, `fontes_json`, `hora_reinicio 09:00` backend (rótulos UI legados `06:00`)) | ✅ Implementado |
...
| Habilitado flag + intervalo 10–9360 clamp na API (select do admin 10–360) + termo livre + fontes/temas `json` configuráveis sem restart | ✅ Implementado |
| Hora reinício `09:00` default do backend (rótulos UI legados `06:00`) configurável + reinício diário `CronTrigger` `reiniciar_banco` `DELETE` + `Zerar agora` (`agregador-reiniciar-agora`) — sem backup | ✅ Implementado |
| Paginação 12 + **busca lado a lado filtro tema** (`pagina` `por_pagina=12` `total_pag` `offset`, `ORDER BY COALESCE(data_publicacao, data_coleta) DESC`, botões `agregador-primeira/anterior/proxima/ultima`, **busca `agregador-busca` `debounce 300` `placeholder "Buscar palavra…"` NFKD 500 `titulo/descricao/fonte/tema` lado a lado filtro tema, sem badge `Coleta:` (botão `Ver puro Noticia` `agregador-ver-puro` removido 20/09/2026 — `/agregador-noticias-puro` por URL direta)**) + grid responsivo **3 colunas desktop → 2 tablet (max-width 1024px) → 1 celular (max-width 640px) via `.grid-noticias {display:grid; grid-template-columns: repeat(3,1fr)}` + media queries, cards de **ALTURA FIXA `300px`** (`260px` no celular, `.card-noticia` — nunca `min/max-height`, para todos os cards da linha ficarem idênticos)** + cards (**miniatura `120×120` `cover` clicável que abre a ampliação em diálogo `60vw×60vh` `contain`** + **logo da fonte como marca d'água `32×32`** no rodapé da imagem, `badge tema`, `ui.link new_tab`, `descricao` **completa com scroll** `overflow-y: auto`, `_tempo_relativo` real) — **redesenho 25/09/2026** + **tela pura** `telas_puro.py` (`/agregador-noticias-puro` `intro-overlay` + `about` `info-list` 12 notícias `COALESCE DESC`, `assets/noticia/` `base.css`/`vendor.css`/`main.css`/`font-awesome`/`micons`/`js`/`images` via `main.py:818` `@app.get("/assets/noticia/{caminho:path}")`) | ✅ Implementado |
