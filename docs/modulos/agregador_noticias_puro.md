...
| Acesso por URL direta `/agregador-noticias-puro` (botão `agregador-ver-puro` removido 20/09/2026) | ✓ | ✓ | — |
...
- **Rotas**: `/agregador-noticias-puro` (`main.py:808-815` `page_agregador_noticias_puro` `pagina_restrita("Agregador de Notícias — Puro", chave_modulo="agregador_noticias")` + `mostrar_tela_pura(...)` `telas_puro.py:48-145`); `/assets/noticia/{caminho:path}` (`main.py:818-831` `servir_assets_noticia`); `/agregador-noticias` **sem** botão `Ver puro Noticia` (removido de `telas.py` 20/09/2026) — `/agregador-noticias-puro` por URL direta.
- **Integração com `/agregador-noticias`**: botão `Ver puro Noticia` (`agregador-ver-puro`) **removido** da barra de filtro+busca de `telas.py` 20/09/2026 — `/agregador-noticias-puro` por URL direta, visível a todos com acesso ao Agregador.
...
# Playwright — tela pura (botão Ver puro removido 20/09/2026; por URL direta)
.venv/bin/pytest assets/test/teste_agregador_noticias.py -k "puro or busca or pure"  # cobre tela pura por URL direta (botão agregador-ver-puro removido 20/09/2026)
```
