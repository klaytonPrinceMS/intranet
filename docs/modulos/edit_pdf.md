...
Criador vigente: `init_db_pdf()` em `manipulador_bd.py:92-123`.
...
⚠️ O `db_criador.py` do módulo é **legado/morto** com esquema divergente — não executar (criaria tabelas erradas no banco central).
...
- `_cfg` depende de `get_config` importado no topo de `db_manipulador.py` (bug real já corrigido: defaults eram usados).
