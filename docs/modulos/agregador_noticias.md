...
- **Grid masonry 3 colunas** (`@ui.refreshable grid()` — `telas.py:163-182`, `limite=45`, `ORDER BY data_coleta DESC`): `column-count: 3; column-gap: 1rem;` + por notícia `break-inside: avoid; margin-bottom: 1rem;` + `_noticia_card(n)` (`telas.py:91-123` + helper `_tempo_relativo` `telas.py:47-89`): `ui.card` `w-full overflow-hidden hover:shadow-lg cursor-pointer` + **miniatura `ui.image(img).classes("w-full h-40 object-cover").props("fit=cover")` quando `imagem_url` presente** (`try` fail-soft, mantém miniatura) + `card_section` com `ui.badge(tema)` (`outline dense`), `ui.link(titulo, target=href, new_tab=True)` (`font-bold`, `word-break`, `new_tab=True` abre externo) , `descricao[:180] + …` (`text-caption text-grey-7`), **`row` apenas tempo relativo `ui.label(_tempo_relativo(data_pub or data_col)).classes("text-caption text-grey-5")` — fonte removida, data absoluta `[:16]` substituída**; `_tempo_relativo` converte `YYYY-MM-DD HH:MM:SS`/`YYYY-MM-DD HH:MM`/`YYYY-MM-DD` (normaliza `T`/`Z`/fração, `delta<0→0`) em `agora` (<60 s) / `X minuto(s) atrás` (<60 min) / `X hora(s) atrás` (<24 h) / `X dia(s) atrás` (<7 d) / `X semana(s) atrás` (<30 d) / `X mês(es) atrás` (<365 d) / `X ano(s) atrás` + fallback `[:16]`; **sem botão "Abrir notícia" — removido em 19/09/2026 (antes `ui.button flat dense` + `window.open` fallback, agora título `ui.link` clicável basta; helper `_abrir` `window.open` permanece mas sem gatilho de botão/card)**. Sem notícia → card `"Nenhuma notícia ainda. Ative a coleta nas Configurações."` + tema.
- **Info TV** (`telas.py:184-187`): `card bg-blue-50 border-blue-200` com `"Integração TV — Filas"` + `"Enquanto aguardam, a TV (/tv) exibe carrossel com título+descrição das últimas notícias via ag.listar_para_tv() (3 colunas aqui, carrossel lá)."`.
- **Helpers**: `_tempo_relativo(data_str)` normaliza ISO flexível + delta relativo; `_noticia_card` normaliza `href` (`#` fallback), sanitiza `titulo[:500]`/`url[:2000]` (`inserir_noticia`), `data-testid` para QA, `notificar` com `timeout` configurável, `ui.colors` do módulo, **fonte armazenada em `tb_noticia.fonte` mas não renderizada no card; sem botão — apenas `ui.image` miniatura quando `imagem_url` + `ui.link(target=href, new_tab=True)` título clicável basta**.
...
- **Card sem fonte e sem botão, só tempo relativo + miniatura + título link** (`telas.py:47-89` `_tempo_relativo` + `91-123` `_noticia_card`): `fonte` continua em `tb_noticia` mas **não exibida no card** — apenas **miniatura `ui.image` quando `imagem_url` + `badge tema + título `ui.link(new_tab=True)` + descrição[:180]… + tempo relativo** (`agora`/<60 s, minutos/<60 min, horas/<24 h, dias/<7 d, semanas/<30 d, meses/<365d, anos); `data_pub or data_col` com `[:16]` removida; parse flexível `T`/`Z`/fração + `delta<0→agora`; **botão "Abrir notícia" removido 19/09/2026 — título clicável basta, mantém 3 colunas masonry**.
- **Anti-disconnect coleta** (`telas.py:137-156`): `async _coletar` com `run.io_bound`, `spinner aria-label=Coletando notícias`, trava `ocupado`, `forcar=True`.
