...
A tela é organizada em **4 abas internas** (`telas.py:mostrar_tela`): **Navegar**, **Fila Renomeação**, **Organizador** (admin) e **Solicitação** (admin) — comum vê só **Navegar**. A antiga aba **Pesquisar** foi incorporada à busca do Navegar (FTS5+levantamento assíncrono) e as **Configurações** vivem no painel admin standalone (`telas_administracao.py:mostrar_administracao`, rota `/admin/empenhos`). O menu lateral global de módulos permanece intacto. A **renomeação automática** pelo monitor pode ser desligada por switch admin sem afetar o processamento manual.
...
- **Busca no levantamento:** `pesquisar_levantamento(termo, limite=100)` (`bd_manipulador.py:2180`) tenta `MATCH` com **prefixo** via `_fts_query_prefixada()` (`'"tok"*'` no último token) e cai no `LIKE` (`nome_arquivo`, `numero_empenho`, `ficha`, `ano`, `conteudo_texto`) em **dois casos**: FTS **indisponível** (Postgres, que ignora `CREATE VIRTUAL TABLE`) **ou FTS sem resultado** (correção de 25/09/2026 — ver abaixo); **retorna também `parcela` e `usuario`** — tuplas `(id, nome, caminho, presente, status, numero, parcela, ficha, ano, usuario)`.

### Busca de 1 letra — corrigido em 25/09/2026

O FTS5 casa **prefixo de token** (`"4"*` acha `4abc`, **não** acha `24`) e o `LIKE` casa **substring** (`%4%` acha `EC_24.pdf`). Antes o `LIKE` só era acionado quando o FTS **lançava exceção** — mas a busca de 1 letra voltava **vazia sem exceção**, então o fallback nunca rodava e a UI (campo `debounce='150'`, rótulo `Pesquisando (FTS5 + conteúdo)…`, docstring "_fts_query_prefixada") **não cumpria a promessa de "1 letra já filtra"**.

**Correção:** o `return` do FTS passou a ficar dentro de `if achados:` e o `LIKE` foi extraído para fora do `except`. Semântica resultante: **prefixo *ou* substring**, com o FTS5 (ordenado por `rank`) como caminho quente e o `LIKE` (`ORDER BY id DESC`) só quando o índice não acha. Coberto por `assets/test/test_navegar_pesquisa_empenhos.py` (20/20 OK), que passou a isolar banco + pasta monitorada em `tempfile` e semear 4 PDFs de `assets/test/pdf/` — a pasta real `doc/` só tem `.gitkeep`, e o teste falhava por falta de massa, não por bug. Detalhe em [Análise do Módulo](../analise_mod_renomear_empenho.md).


## Funcionalidades

- **4 abas internas em `telas.py` + painel admin standalone** (barra de abas pelo helper padrão `aba_modulo.menu_modulo` desde 25/09/2026 — ver abaixo; painel admin standalone no padrão `mod_solicita_impressao/telas.py`):
  ...
  - **Pesquisar** (incorporada ao Navegar): busca FTS5 (`MATCH`, fallback `LIKE`) + levantamento; a aba própria foi removida — a busca global de conteúdo vive no campo de pesquisa do Navegar.
  - **Configurações** (admin, painel standalone `telas_administracao.py` em `/admin/empenhos`): pastas monitoradas (multi-pasta, UNC), aparência, template de nome, campos de busca, quarentena e regras regex — **sem aba de Auditoria** (removida; auditoria vive no menu Auditoria via `audit_log` central).
...
- **Pesquisa na aba Navegar — FTS5 assíncrona com prefixo** (`telas.py:412`):
  - Handler `async _filtrar()` (`telas.py:471`) dispara a cada `update:model-value` do `ui.input` `data-testid=empenhos-navegar-pesquisa`; **1 letra já filtra** — via `_fts_query_prefixada()` (último token vira `'"tok"*'`) **e, quando o FTS não acha nada, via `LIKE` por substring** (correção de 25/09/2026, ver acima).
...
- **Responsividade (RNF-UI-01, 09/2026 — auditado 320/768/1024 `kbp-web-design`)**: proposta P0/P1/P2 por `container`/`row`/`grid` — filtros/busca `flex-wrap` `flex-1 min-w`, tabelas `overflow-x-auto`, `scroll_area` altura explícita, grids `grid-cols-1 sm:grid-cols-2 md:grid-cols-3`, dialogs `w-full max-w`; cabeçalho `flex-wrap` `truncate`.
- **Barra de abas no helper padrão (25/09/2026)**: `telas.py:186` passou de `ui.tabs()`/`ui.tab()` **manuais** para `tabs_el = menu_modulo(_abas)`, e o dicionário paralelo `_icones` (redundante — o ícone já vinha dentro do 3º elemento de cada tupla de `_abas` e era ignorado pelo `ui.tab`) foi **removido**. Antes, `menu_modulo` já estava **importado e sem uso**, o que fazia o código de produção violar a própria regra cobrada por `verifica_ui_comum.py`. Efeito colateral intencional: o helper monta as abas com **ícone acima do rótulo** (`ui.tabs().classes("w-full")`, sem `dense inline-label`).
...
O padrão do módulo é **PIC — Quasar nativo suave (sem framework)** (`mod_renomear_empenho/visual.py:MODELO_PADRAO="pic"`, `CHAVE_CONFIG="empenhos_modelo_visual"`). `visual.FRAMEWORKS` lista 12 CSS novos (`spectre`, `chota`, `milligram`, `skeleton`, `water`, `mvp`, `tachyons`, `uikit`, `foundation`, `semantic`, `materialize`, `primer`); `VALIDOS=("pic",)+FRAMEWORKS`. Ramos `bootstrap`/`hibrido` ainda referenciados em `telas.py`/`telas_administracao.py` são legado inativo (fora de `VALIDOS`, caem no PIC). Não há `CHAVE_POR_ROTA` neste módulo — a chave de rota é fixa `empenhos` (`/renomear-empenho`).
...
- **Onde:** totalmente à direita do `menu_mod` (row `justify-between flex-nowrap`: barra de abas via `menu_modulo(_abas)` à esquerda e botão `shrink-0 ml-auto` à direita), rótulo "Gerar 25 (TEMP)" ícone `science` variante `secundario` `data-testid=empenhos-gerar-25-temp`.
...
- `assets/test/test_navegar_pesquisa_empenhos.py` — **20/20 OK**: navegação, FTS5, `_fts_query_prefixada` de 1 letra, **busca de 1 letra no levantamento** (com massa semeada em pasta temporária), lote async sem disconnect, gate de automação, quarentena/fila com testid, template de nome e correção EE `str→int`.
...
- `tb_levantamento_fts` é **VIRTUAL TABLE FTS5 exclusiva do SQLite** — no Postgres o `CREATE VIRTUAL TABLE` é ignorado e `pesquisar_levantamento` usa `LIKE`; comportamento esperado via `banco_conexao`. Esse `LIKE` **não é fallback dispensável**: desde 25/09/2026 ele é também o **complemento de substring** do SQLite (é o que faz a busca de 1 letra funcionar).
- **`overflow-x-auto` perdido na barra de abas (25/09/2026)** — a versão manual de `ui.tabs()` trazia `min-w-0 flex-1 overflow-x-auto`; `menu_modulo` devolve `w-full` **sem** a rolagem horizontal. Com 4 abas admin cabe em ~320 px, mas **vale reverificar no RNF-UI-01** se o menu ganhar abas. Alternativa sem tocar no helper: envolver o `tabs_el` num `ui.element("div").classes("min-w-0 overflow-x-auto")`.
- **`ui.notify` cru segue sendo a maioria das notificações** (25/09/2026): 117 ocorrências em `telas.py` + 23 em `telas_administracao.py`, contra a regra do AGENTS.md §5.1. O `mod_edit_pdf` foi migrado para `tema_modulo.notificar()` no mesmo dia; este módulo ainda não — é tarefa própria, com guarda de QA por arquivo.
