...
- **Menu lateral (drawer)**: item **"Home"** no topo (ícone `home`) navega para `/` (`layout_tela.py:138-143`); **sem labels de seção** desde 06/09 (removidos "PÁGINA INICIAL"/"MÓDULOS"/"SISTEMA" — apenas separadores entre os grupos); no rodapé do drawer, o item **"Administração"** (antes "Configurações", ícone `admin_panel_settings`, `layout_tela.py:184-200`) é **contextual/dinâmico**: dentro de um módulo navega para `/admin/{chave_modulo}` (painel standalone daquele módulo); no Home (`chave_modulo=None`) navega para `/configuracoes` (administração geral do sistema). O tooltip acompanha o contexto: "Configurações de {nome_modulo}" vs "Configurações gerais do sistema" (`layout_tela.py:190,193`). Exclusivo do `administrador_geral`; abaixo dele, "Documentação" abre `/documentacao` em nova aba.
