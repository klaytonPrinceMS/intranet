...
|:---|:---|:---|:---|:---|
| intranet (núcleo) | ~~3 connects crus sem WAL; zero `busy_timeout` no repo; WAL OK no quente~~ → **SUPERADO 25/09/2026** | `mod_intranet/banco_conexao.py:78` · `:106` · `:705` · `mod_intranet/ativacao.py:507` (TODOS com WAL hoje) · WAL quente em `banco_conexao.py:114`, `crud_base.py:121,144`, `repositorio.py:164` | Nenhuma ação pendente — os connects migraram de linha e já aplicam `PRAGMA journal_mode=WAL`; `busy_timeout` foi adotado nos módulos | Superada |
...
- **Crus sem WAL: SUPERADO (25/09/2026).** O achado original ("`banco_conexao.py:74,90` + `ativacao.py:502` usam `sqlite3.connect` sem pragmas") **não vale mais**: as linhas migraram para `:78`, `:106`, `:705` e `ativacao.py:507`, e todas aplicam `PRAGMA journal_mode=WAL`. Verifique no código antes de agir neste item.
- **`busy_timeout` adotado nos módulos** (não é mais "zero no repo"): `mod_agregador_noticias/bd_manipulador.py:75`, `mod_filas/bd_manipulador.py:72`, e herdado via `banco_conexao.conexao()` pelos demais.
