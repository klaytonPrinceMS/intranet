...
| `/agregador-noticias` | `@ui.page` | `mod_agregador_noticias` | liberados (`agregador_noticias`, `_pode_ver` + `validar_acesso_modulo`) — grid responsivo `grid-noticias` 3→2→1 (1024px/640px) + `.card-noticia` 160–220px (média 91 notícias) `agregador-filtro-tema` + `agregador-busca` `debounce 300` (botão `agregador-ver-puro` removido 20/09/2026 — `/agregador-noticias-puro` por URL direta), paginação 12 `agregador-primeira/anterior/proxima/ultima` | `main.py:794` `page_agregador_noticias` → `mod_agregador_noticias/telas.py:mostrar_tela` |
