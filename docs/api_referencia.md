...
| `/agregador-noticias` | `@ui.page` | `mod_agregador_noticias` | liberados (`agregador_noticias`, `_pode_ver` + `validar_acesso_modulo`) — grid responsivo `grid-noticias` 3→2→1 (1024px/640px) + `.card-noticia` 160–220px (média 91 notícias) `agregador-filtro-tema` + `agregador-busca` `debounce 300` + `agregador-ver-puro` → `/agregador-noticias-puro`, paginação 12 `agregador-primeira/anterior/proxima/ultima` | `main.py:794` `page_agregador_noticias` → `mod_agregador_noticias/telas.py:mostrar_tela` |
...
| `/assets/noticia/{caminho:path}` | `@app.get` (FastAPI) | `mod_agregador_noticias` | público (CSS/JS/fonts/images da tela pura) — path-traversal safe `normpath` + `startswith(base)` + `mimetypes.guess_type` → `FileResponse`, `404` se fora ou não existe | `main.py:818` `servir_assets_noticia` |
