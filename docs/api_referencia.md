...
| `/agregador-noticias` | `@ui.page` | `mod_agregador_noticias` | liberados (`agregador_noticias`, `_pode_ver` + `validar_acesso_modulo`) — grid responsivo `grid-noticias` 3→2→1 (1024px/640px) + `.card-noticia` de **altura fixa `300px`** (miniatura `120×120` ampliável em diálogo, `data-testid=agregador-ampliar-<id>`) `agregador-filtro-tema` + `agregador-busca` `debounce 300` (botão `agregador-ver-puro` removido 20/09/2026 — `/agregador-noticias-puro` por URL direta), paginação 12 `agregador-primeira/anterior/proxima/ultima` | `main.py:794` `page_agregador_noticias` → `mod_agregador_noticias/telas.py:mostrar_tela` |
| `/agregador-noticias-puro` | `@ui.page` | `mod_agregador_noticias` | liberados (`agregador_noticias`, mesmo gate) — pura estilo Noticia `intro-overlay` + `about` `info-list` com cards atuais (3 cols, `fonte_icon 16×16` + `imagem 30×30`, tempo relativo, 12 notícias `listar_noticias(limite=12)`) + `assets/noticia/` `base.css`/`vendor.css`/`main.css`/`font-awesome`/`micons`/`js`/`images` via `main.py:818` | `main.py:808` `page_agregador_noticias_puro` → `mod_agregador_noticias/telas_puro.py:mostrar_tela_pura` |
| `/api/attachments/{caminho:path}` | `@app.get` (FastAPI) | `mod_agregador_noticias` (+ `mod_blog`) | público (fallback silencioso **antes de `/assets/noticia`**) — `favicon.png` placeholder `assets/noticia/favicon.png` se caminho termina em `-w280-h168-p-df/-w280-h168/.png/.jpg/.jpeg` senão `404`; evita log spam `http://localhost:8080/api/attachments/CC8... not found` no boot `NiceGUI ready`; docstring bilíngue EN/PT-BR; TV/Blog sem imagem quebrada; compatível, sem quebrar coleta — `bd_manipulador.inserir_noticia` filtra `"/api/attachments"` relativo/localhost → `""` (absoluto `news.google.com/api/attachments` preservado — thumbnail real) | `main.py:820` `fallback_attachments` — `if caminho.endswith(...) → FileResponse(favicon.png)` else `Response(404)` |
| `/assets/noticia/{caminho:path}` | `@app.get` (FastAPI) | `mod_agregador_noticias` | público (CSS/JS/fonts/images da tela pura) — path-traversal safe `normpath` + `startswith(base)` + `mimetypes.guess_type` → `FileResponse`, `404` se fora ou não existe | `main.py:834` `servir_assets_noticia` |
