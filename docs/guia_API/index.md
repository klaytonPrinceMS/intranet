...
| `/filas` | `mod_filas` | `filas` | liberados (`Chamar próximo` + preview) |
| `/tv` | `mod_filas` (`mostrar_tv`) | — (pública) | **pública sem login** (TV full-screen, auto-refresh 3s + beep) |
...
- `db_mod_filas.db`: `tb_fila` (`Geral` seed `A000`/`ativa`/`01`) + `tb_chamada` (`fila_id` FK CASCADE, `senha`, `guiche` snapshot) + `tb_config_filas` (`filas_modo_tv`, `filas_senha_prefixo`, `filas_guiche_padrao`).
