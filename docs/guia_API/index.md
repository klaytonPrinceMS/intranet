...
| `/filas` | `mod_filas` | `filas` | liberados (multi-filas por local, `criado_por` isolado + `tb_fila_acesso`, `Cadastro de fila` recolhível + Editar no painel + `Chamar próximo` + `Avançar`, `delay=1000`, admin via hambúrguer) |
| `/tv` | `mod_filas` (`mostrar_tv` com `?grupo=`) | — (pública) | **pública sem login** (TV compartilhada `?grupo=xxx` + `?etapa=` por sala, auto-refresh 3s + bip 880Hz + voz `pt-BR` serializada por claim na ordem `voz_ordem` + ducking à metade (volume 40) + papel de fundo + playlist `/midia_filas` + carrossel até 200 censura-filtrado) |
| `/tv/{fila_id}` | `mod_filas` (`mostrar_tv` isolada) | — (pública) | **pública sem login** (TV isolada por fila, voz serializada por claim na ordem `voz_ordem`, ducking à metade, papel de fundo absoluto só na área de mídia) |
| `/midia_filas/*` | `mod_filas` (`PASTA_MIDIA` `mod_filas/midia`) | — | público (áudios MP3 / vídeos MP4 / fotos por fila + ambiente global, `datahora_nomeFila.ext` sempre via `renomear_arquivos_fila`, volume 40) |
| `/lista-telefonica` | `mod_lista_telefonica` | `lista_telefonica` | liberados (organograma `Secretaria→Setor→Subsetor`, busca, `tel:`) |
| `censura` | `mod_intranet/censura.py` | `conteudo_palavras_bloqueadas` | central (Blog `criar/atualizar` bloqueia + Agregador `inserir` descarta (`fonte_icon`/`imagem` extração `faviconV2`/`api/attachments`) / `listar_para_tv` filtra `limite=200` (`fonte_icon` incluso) / `limpar_censuradas` remove; `lower+NFD` sem acentos) |
...
- `db_mod_filas.db`: `tb_fila` SEM seed (nasce vazio + `prefixo/inicio→fim` `fim=0` infinito + `criado_por` + `tv_grupo` slug + voz `voz_ordem`, `tb_fila_etapa`) + `tb_chamada` (`fila_id` FK CASCADE, `senha`, `guiche` snapshot + `paciente_nome/etapa_nome/fila_nome` + prioridade/manchester) + `tb_fila_nomes` (lista única) + `tb_midia` (por fila + global + `fundo`, volume 40, `/midia_filas/*` `datahora_nomeFila` sempre) + `tb_tv_estado` (claim) + `tb_fila_acesso` + `tb_config_filas` + `PASTA_MIDIA` `mod_filas/midia` + notícias até 200 censura-filtradas em `listar_para_tv`.
