...
| **Filas (TV)** | `filas` | `/filas` + `/tv?grupo=` (compartilhada) + `/tv/{fila_id}` (isolada) + `?etapa=` (por sala) | `db_mod_filas.db` | multi-filas sem seed (`prefixo/inicio→fim` `fim=0` infinito, `criado_por`, `tv_grupo` slug, `voz_ordem` + `tb_fila_etapa`/`tb_fila_nomes`/`tb_midia` com `fundo`, isolamento por criador/`tb_fila_acesso` vs `administrador_geral`, TV isolada vs compartilhada vs etapa, bip+voz claim na ordem + ducking metade (volume 40) + fundo, playlist `/midia_filas` (`datahora_nomeFila` sempre), Editar no painel + Acesso, notícias 200, censura filtrada) |
...
| **Agregador de Notícias** | `agregador_noticias` | `/agregador-noticias` + `/agregador-noticias-puro` (pura) + `/assets/noticia/*` + `/admin/agregador_noticias` | `db_mod_agregador_noticias.db` | **novo** (19/09/2026) — multi-fonte `httpx+parsel` (Google/BBC/JFP/RSS) espelhando `klaytonPrinceMS/Noticia`, termo livre + fontes configuráveis, 10–9360 min na API (select do admin 10–360) **barra filtro tema + busca lado a lado** `agregador-busca` `debounce 300` `placeholder "Buscar palavra…"` NFKD (**sem badge `Coleta:`** removido a169c9a) (botão `agregador-ver-puro` removido 20/09/2026 — `/agregador-noticias-puro` por URL direta), grid responsivo **3→2→1 via `.grid-noticias {display:grid; grid-template-columns: repeat(3,1fr)}` + media queries 1024px/640px + `.card-noticia {height: 300px}` (altura FIXA, `260px` no celular)** `ui.link(target=href, new_tab=True)` + **miniatura `120×120` ampliável em diálogo** (`imagem_url`) + **logo da fonte como marca d'água `32×32`** (`fonte_icon_url` `faviconV2`; ícone `16×16` ao lado do badge quando não há imagem) + **descrição completa com scroll**, paginação 12 + busca 500 NFKD, **tela pura** `telas_puro.py` `intro-overlay` + `about` `info-list` (12 notícias `COALESCE DESC`, `assets/noticia/` `base.css`/`vendor.css`/`main.css`/`font-awesome`/`micons`/`js`/`images` via `main.py:818` `@app.get("/assets/noticia/{caminho:path}")`), 24h `limpar_antigas`/`reiniciar_banco` `09:00` backend (rótulos UI `06:00`) `CronTrigger`, TV `listar_para_tv` (`fonte_icon` incluso, censura filtrada) + censura central `conteudo_palavras_bloqueadas` + `assets/noticia/` 4.9M servido via rota estática |
...
| Agregador de Notícias | ver 3 colunas + **filtro tema + busca lado a lado** `agregador-busca` `debounce 300` NFKD + abrir link externo pelo título e **ampliar a imagem da miniatura `120×120` em diálogo** (marca d'água `32×32` da fonte, sem badge `Coleta:`, censura filtra `listar_para_tv`) | configurar habilitado/intervalo/termo/fontes/temas + censura (`agregador-palavras-bloqueadas`) + **busca NFKD** | tudo + `Coletar agora` + `Remover já censuradas` |
...
- Tentativa sem permissão gera `acesso_negado` na trilha de auditoria (`mod_intranet/telas.pagina_restrita` — choke point único).
