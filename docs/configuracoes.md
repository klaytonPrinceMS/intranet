...
| `db_mod_agregador_noticias.db` | agregador de notícias | `mod_agregador_noticias/bd_manipulador.py:21` — `tb_noticia` (titulo/fonte/tema/url/imagem/descricao/data_publicacao/data_coleta) + `TEMAS_PADRAO` 9 + `FONTES_PADRAO` 3 (Google/BBC), coleta `httpx+parsel` paginada 10 (`por_pagina=10` `COALESCE DESC` `Primeira/Anterior/Próxima/Última` `agregador-primeira/anterior/proxima/ultima`, `30x30` antes do título), **sem backup** (`MAPA_BACKUPS` sem `agregador_noticias`), **reinício diário `06:00` configurável** (`agregador_noticias_hora_reinicio` `HH:MM` + `reiniciar_banco` `DELETE` + `CronTrigger`) |
...
| Agregador de Notícias | `agregador_noticias_habilitado` (`0`), `agregador_noticias_intervalo_min` (`60` clamp 10–360), `agregador_noticias_termo_pesquisa` (`""`), `agregador_noticias_temas_json` (`TEMAS_PADRAO` 9), `agregador_noticias_fontes_json` (`FONTES_PADRAO` 3), **`agregador_noticias_hora_reinicio` (`06:00` `HH:MM` 00–23:00–59)**, tema `agregador_noticias_*` + **censura** `conteudo_palavras_bloqueadas` (central, `limpar_censuradas()` sem audit, `listar_para_tv` filtra) + **paginação 10** (`por_pagina=10` `COALESCE DESC` `Primeira/Anterior/Próxima/Última` `agregador-primeira/anterior/proxima/ultima`, `30x30` antes do título, sem fonte/card-TV) + **reinício diário `06:00` `CronTrigger`** (`obter_hora_reinicio`/`definir_hora_reinicio` audit + `reiniciar_banco` `DELETE` audit, `reconfigurar_agregador_noticias` `CronTrigger`) **sem backup** (`MAPA_BACKUPS` sem `agregador_noticias`) + **auditoria só quem alterou o quê** (`definir_*`/`reiniciar_banco`, sem postagens) | — | **paginação 10** + **30x30** + **reinício diário `06:00` `06:00` configurável + `Zerar agora`** + sem backup (banco reciclado) + `httpx+parsel` (Google/BBC/JFP/RSS) + 3 colunas masonry `COALESCE DESC` + TV `listar_para_tv` filtrado + censura compartilhada com Blog — ver [Módulo Agregador](modulos/agregador_noticias.md) |
