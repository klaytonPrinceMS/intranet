...
12. `init_db()` agregador de notícias (`mod_agregador_noticias/bd_manipulador.py:209`) → `db_mod_agregador_noticias.db` + `tb_noticia` + `TEMAS_PADRAO` 9 + `FONTES_PADRAO` 3 (idempotente, seeds `tb_config` `habilitado=0`/`intervalo=60`/`termo=""`/`hora_reinicio=06:00` + `reiniciar_banco` `DELETE`; **sem backup** `MAPA_BACKUPS` sem `agregador_noticias`; agendadores `agregador_coleta` `10–360 min` + `agregador_reinicio` `CronTrigger` `06:00` configurável).
