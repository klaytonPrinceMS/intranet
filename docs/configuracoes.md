...
12. `init_db()` agregador de notícias (`mod_agregador_noticias/bd_manipulador.py:209`) → `db_mod_agregador_noticias.db` + `tb_noticia` + `TEMAS_PADRAO` 9 + `FONTES_PADRAO` 15 (3 + 8 RSS + 4 cobertura, migração V1/V2; idempotente, seeds `tb_config` `habilitado=0`/`intervalo=60`/`termo=""`/`hora_reinicio=06:00` + `reiniciar_banco` `DELETE`; **sem backup** `MAPA_BACKUPS` sem `agregador_noticias`; agendadores `agregador_coleta` `10–360 min` + `agregador_reinicio` `CronTrigger` `06:00` configurável).
...
| Agregador de Notícias | `agregador_noticias_habilitado` (`0`), `agregador_noticias_intervalo_min` (`60` clamp 10–360), `agregador_noticias_termo_pesquisa` (`""`), `agregador_noticias_temas_json` (`TEMAS_PADRAO` 9), `agregador_noticias_fontes_json` (`FONTES_PADRAO` 15: 3 + 8 RSS + 4 cobertura), **`agregador_noticias_hora_reinicio` (`06:00` `HH:MM` 00–23:00–59)**, tema `agregador_noticias_*` + **censura** `conteudo_palavras_bloqueadas` (central, `limpar_censuradas()` sem audit, `listar_para_tv` filtra `fonte_icon` incluso) + **paginação 12 + busca + grid responsivo 3→2→1 + altura 160–220px** (`por_pagina=12` `COALESCE DESC` `Primeira/Anterior/Próxima/Última` `agregador-primeira/anterior/proxima/ultima` + `agregador-busca` `debounce 300` NFKD lado a lado filtro tema, **sem badge `Coleta:`**, **grid `.grid-noticias` 3→2→1 + `.card-noticia` 160–220px (média 91 notícias: título 79/75, descrição 34, 87.9% imagem, 24% fonte_icon) + ícone fonte 16×16 + imagem 30×30 lado a lado antes do título**) + **reinício diário `06:00` `CronTrigger`** (`obter_hora_reinicio`/`definir_hora_reinicio` audit + `reiniciar_banco` `DELETE` audit, `reconfigurar_agregador_noticias` `CronTrigger`) **sem backup** (`MAPA_BACKUPS` sem `agregador_noticias`) + **auditoria só quem alterou o quê** (`definir_*`/`reiniciar_banco`, sem postagens) | — | **paginação 12 + busca + grid 3→2→1 + altura 160–220px + `16×16`+`30×30`** + **reinício diário `06:00` `06:00` configurável + `Zerar agora`** + sem backup (banco reciclado) + `httpx+parsel` (Google/BBC/JFP/RSS) + grid responsivo `COALESCE DESC` + **busca NFKD lado a lado** + TV `listar_para_tv` (`fonte_icon` incluso) filtrado + censura compartilhada + assets `assets/noticia/` (tema Noticia 4.9M) — ver [Módulo Agregador](modulos/agregador_noticias.md) |
