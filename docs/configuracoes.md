...
| E-mail/SMTP | `smtp_*` | — | credenciais (cartão "E-mail / SMTP") |
| Logs | `log_ativo`, `log_nivel`, `log_rotacao` (`1 month`), `log_retencao` (`4 months`) | — | observabilidade loguru |

> Os padrões de aparência vivem em `PADRAO_CONFIG` (`mod_intranet/conexao_bd.py:13-24`) e são restaurados via tela de configurações (cartões com "Restaurar padrão").
