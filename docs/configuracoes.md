...
| `db_mod_filas.db` | filas — multi-filas sem seed (`tb_fila` com `prefixo/inicio→fim` `fim=0` infinito + `criado_por` + `tv_grupo` slug 2–40 + voz `voz_nome/senha/destino/guiche/fila/hora` + `voz_ordem` `fila,senha,nome,destino,guiche` via `normalizar_voz_ordem` + `voz_repetir/intervalo` + textos `tv_*` + `prio_turno`, `tb_fila_etapa`, `tb_chamada` com `paciente_nome/etapa_nome` + `prioridade/manchester`, `tb_fila_nomes` 1 lista/fila com tags `#prioridade #manchester #etapa` via `csv_para_tags`, `tb_midia` por fila + ambiente global NULL (`volume` 40 + `duracao` + `slot` + `duracao_real` mutagen + `fundo` papel de fundo via `set_midia_fundo`), `tb_tv_estado` claim+remoto, `tb_fila_acesso` (Editar no painel + Acesso)) + `/tv/{id}` isolada + `/tv?grupo=` compartilhada + `?etapa=` por sala + mídia `/midia_filas` (`datahora_nomeFila.ext` sempre, reaplicado por `renomear_arquivos_fila` ao renomear) + voz serializada por claim na ordem + ducking à metade + hora cheia/meia + `delay=1000` + cards recolhíveis (admin via hambúrguer) + carrossel Agregador `listar_para_tv(limite=200)` filtra `conteudo_palavras_bloqueadas` | `mod_filas/bd_manipulador.py` — `PASTA_MIDIA` `mod_filas/midia` + `/midia_filas/*` + `mutagen>=1.45` |
...
| `db_mod_agregador_noticias.db` | agregador de notícias | `mod_agregador_noticias/bd_manipulador.py:21` — `tb_noticia` (titulo/fonte/tema/url/**imagem+ícone_fonte**/descricao/data_publicacao/data_coleta, `fonte_icon_url` `faviconV2` (marca d'água `32×32` / ícone `16×16` quando não há imagem) + `imagem_url` miniatura `120×120` `/api/attachments`) + `TEMAS_PADRAO` 9 + `FONTES_PADRAO` 15 (3 Google/BBC + 8 RSS V1 + 4 cobertura V2; migração V1/V2 no `init_db` 20/09/2026), coleta `httpx+parsel` grid responsivo **3→2→1 + paginado 12** + **busca lado a lado filtro tema** (`por_pagina=12` `COALESCE DESC` `Primeira/Anterior/Próxima/Última` `agregador-primeira/anterior/proxima/ultima` + `agregador-busca` `debounce 300` `placeholder "Buscar palavra…"` NFKD, **sem badge `Coleta:`** removido, **grid `.grid-noticias {display:grid; grid-template-columns: repeat(3,1fr)}` + `@media 1024px→2 640px→1` + `.card-noticia {height: 300px}` (altura FIXA, `260px` no celular) + miniatura `120×120` ampliável em diálogo + marca d'água `32×32` da fonte + descrição completa com scroll (botão `Ver puro Noticia` `agregador-ver-puro` removido 20/09/2026 — `/agregador-noticias-puro` por URL direta)**), **tela pura** `telas_puro.py` (12 notícias `info-list`, `assets/noticia/` `base.css`/`vendor.css`/`main.css`/`font-awesome`/`micons`/`js`/`images` via `main.py:818` `@app.get("/assets/noticia/{caminho:path}")` path-traversal safe), **sem backup** (`MAPA_BACKUPS` sem `agregador_noticias`), **reinício diário `06:00` configurável** (`agregador_noticias_hora_reinicio` `HH:MM` + `reiniciar_banco` `DELETE` + `CronTrigger`) + `assets/noticia/` 4.9M servido via rota estática + `mod_filas/midia/` (`PASTA_MIDIA` `/midia_filas`) |
...
10. `init_db()` filas (`mod_filas/bd_manipulador.py`) → `db_mod_filas.db` **SEM seed (projeto nasce sem filas)** + 8 tabelas (`tb_fila` voz `voz_ordem` + textos + `tb_fila_etapa` + `tb_chamada` prioridade/manchester + `tb_fila_nomes` + `tb_midia` por fila/global + `fundo` + volume 40 + `tb_tv_estado` + `tb_fila_acesso` + `tb_config_filas`) + migração slug `tv_grupo` + `mutagen` duração real + `renomear_arquivos_fila` + limpeza `stage_*` 24h.
...
12. `init_db()` agregador de notícias (`mod_agregador_noticias/bd_manipulador.py:209`) → `db_mod_agregador_noticias.db` + `tb_noticia` + `TEMAS_PADRAO` 9 + `FONTES_PADRAO` 15 (3 + 8 RSS + 4 cobertura, migração V1/V2; idempotente, seeds `tb_config` `habilitado=0`/`intervalo=60`/`termo=""`/`hora_reinicio=09:00` (backend; rótulos UI legados `06:00`) + `reiniciar_banco` `DELETE`; **sem backup** `MAPA_BACKUPS` sem `agregador_noticias`; agendadores `agregador_coleta` `10–9360 min` na API (select do admin 10–360) + `agregador_reinicio` `CronTrigger` `09:00` backend (rótulos UI `06:00`) configurável).
...
|:---|:---|:---|:---|
| Censura (central) | `conteudo_palavras_bloqueadas` | `""` | lista CSV de palavras bloqueadas em títulos (Blog `criar/atualizar` + Agregador `inserir`/`listar_para_tv`/`limpar_censuradas`); normalização `lower+NFD` sem acentos (`suicidio`→`suicídio`), substring insensível; `mod_intranet/censura.py` `CHAVE_CONFIG` |
...
| Blog | `blog_modo_exibicao` (**`carrossel` padrão com 3 básicas 18/09/2026**, `historico`/`unica`/`carrossel`), `blog_postagem_unica_id`, `blog_carrossel_postagens_ids` (CSV), `blog_carrossel_tempo` (10 s), `blog_largura_imagem`, `blog_tags_permitidas`, `blog_texto_header` + tema `blog_*` + **censura** `conteudo_palavras_bloqueadas` (central, `mod_intranet/censura.py`, `data-testid=blog-palavras-bloqueadas`, `titulo_bloqueado` antes de `nh3`) | — | comportamento/aparência + censura — `init_db` garante carrossel 3 básicas via `_garantir_carrossel_padrao` (`bd_manipulador.py:241`) |
...
| Filas | `filas_modo_tv` (1), `filas_senha_prefixo` (A), `filas_guiche_padrao` (01), tema `filas_*` + multi-filas sem seed (`tv_grupo` slug + voz `voz_*` + `voz_ordem` `fila,senha,nome,destino,guiche` via `normalizar_voz_ordem` + textos `tv_*`, `tb_fila_etapa`/`tb_fila_nomes` (`csv_para_tags`)/`tb_midia` por fila+global (`volume` 40 + `fundo` + `datahora_nomeFila` sempre via `renomear_arquivos_fila`)/`tb_tv_estado` claim/`tb_fila_acesso` (Editar no painel + Acesso), `/tv/{id}` vs `/tv?grupo=` vs `?etapa=`, `/midia_filas` + `midias_para_tv` (áudio global em todas), claim sem cortar+ducking metade+hora, `delay=1000` + cards recolhíveis, admin via hambúrguer, censura filtrada, notícias 200) | — | multi-filas por local + TV isolada/compartilhada/por sala + mídia/fundo + lista única — ver [Módulo Filas](modulos/filas.md) |
...
| Agregador de Notícias | `agregador_noticias_habilitado` (`0`), `agregador_noticias_intervalo_min` (`60` clamp 10–9360 na API; select do admin 10–360), `agregador_noticias_termo_pesquisa` (`""`), `agregador_noticias_temas_json` (`TEMAS_PADRAO` 9), `agregador_noticias_fontes_json` (`FONTES_PADRAO` 15: 3 + 8 RSS + 4 cobertura), **`agregador_noticias_hora_reinicio` (`09:00` backend, rótulos UI legados `06:00`, `HH:MM` 00–23:00–59)**, tema `agregador_noticias_*` + **censura** `conteudo_palavras_bloqueadas` (central, `limpar_censuradas()` sem audit, `listar_para_tv` filtra `fonte_icon` incluso) + **paginação 12 + busca + grid responsivo 3→2→1 + altura 160–220px** (`por_pagina=12` `COALESCE DESC` `Primeira/Anterior/Próxima/Última` `agregador-primeira/anterior/proxima/ultima` + `agregador-busca` `debounce 300` NFKD lado a lado filtro tema, **sem badge `Coleta:`**, **grid `.grid-noticias` 3→2→1 + `.card-noticia` 160–220px (média 91 notícias: título 79/75, descrição 34, 87.9% imagem, 24% fonte_icon) + ícone fonte 16×16 + imagem 30×30 lado a lado antes do título**) + **reinício diário `09:00` backend (rótulos UI `06:00`) `CronTrigger`** (`obter_hora_reinicio`/`definir_hora_reinicio` audit + `reiniciar_banco` `DELETE` audit, `reconfigurar_agregador_noticias` `CronTrigger`) **sem backup** (`MAPA_BACKUPS` sem `agregador_noticias`) + **auditoria só quem alterou o quê** (`definir_*`/`reiniciar_banco`, sem postagens) | — | **paginação 12 + busca + grid 3→2→1 + altura 160–220px + `16×16`+`30×30`** + **reinício diário `09:00` backend (`06:00` nos rótulos UI legados) configurável + `Zerar agora`** + sem backup (banco reciclado) + `httpx+parsel` (Google/BBC/JFP/RSS) + grid responsivo `COALESCE DESC` + **busca NFKD lado a lado** + TV `listar_para_tv` (`fonte_icon` incluso) filtrado + censura compartilhada + assets `assets/noticia/` (tema Noticia 4.9M) — ver [Módulo Agregador](modulos/agregador_noticias.md) |
...
Ver também: [Arquitetura — Guarda de página e layout](arquitetura.md#guarda-de-pagina-e-layout) e [Análise do Núcleo](analise_mod_intranet.md#guarda-de-pagina-telaspagina_restrita).
