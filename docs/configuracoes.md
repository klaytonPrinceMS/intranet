...
| `db_mod_filas.db` | filas — multi-filas (`tb_fila` com `endereco/prefixo/inicio→fim` `fim=0` infinito + `criado_por` + `tv_grupo`, `tb_fila_etapa`, `tb_chamada` com `paciente_nome/etapa_nome`, `tb_midia` global) + `/tv/{id}` isolada + `/tv?grupo=` compartilhada + mídia `/midia_filas` + carrossel Agregador `listar_para_tv` filtra `conteudo_palavras_bloqueadas` | `mod_filas/bd_manipulador.py:15` — `PASTA_MIDIA` `mod_filas/midia` + `/midia_filas/*` |
...
| Filas | `filas_modo_tv` (1), `filas_senha_prefixo` (A), `filas_guiche_padrao` (01), tema `filas_*` + multi-filas (`endereco/prefixo/inicio→fim` infinito, `criado_por`, `tv_grupo`, `tb_fila_etapa`/`tb_midia`, `/tv/{id}` vs `/tv?grupo=`, `/midia_filas`, censura filtrada) | — | multi-filas por local + TV isolada/compartilhada + mídia elevador/propaganda — ver [Módulo Filas](modulos/filas.md) |
