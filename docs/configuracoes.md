...
| `db_mod_filas.db` | filas — multi-filas sem seed (`tb_fila` com `endereco/prefixo/inicio→fim` `fim=0` infinito + `criado_por` + `tv_grupo` slug 2–40 + voz `voz_nome/senha/destino/guiche/fila/hora` + `voz_repetir/intervalo` + textos `tv_*` + `prio_turno`, `tb_fila_etapa`, `tb_chamada` com `paciente_nome/etapa_nome` + `prioridade/manchester`, `tb_fila_nomes` 1 lista/fila com tags `#prioridade #manchester #etapa`, `tb_midia` por fila + ambiente global NULL (`volume` 20 + `duracao` + `slot` + `duracao_real` mutagen), `tb_tv_estado` claim+remoto, `tb_fila_acesso`) + `/tv/{id}` isolada + `/tv?grupo=` compartilhada + `?etapa=` por sala + mídia `/midia_filas` (`datahora_nomeFila.ext`) + voz serializada claim + ducking 50% + hora cheia/meia + carrossel Agregador `listar_para_tv(limite=200)` filtra `conteudo_palavras_bloqueadas` | `mod_filas/bd_manipulador.py:113` — `PASTA_MIDIA` `mod_filas/midia` + `/midia_filas/*` + `mutagen>=1.45` |
...
10. `init_db()` filas (`mod_filas/bd_manipulador.py:113`) → `db_mod_filas.db` **SEM seed (projeto nasce sem filas)** + 8 tabelas (`tb_fila` voz/textos + `tb_fila_etapa` + `tb_chamada` prioridade/manchester + `tb_fila_nomes` + `tb_midia` por fila/global + `tb_tv_estado` + `tb_fila_acesso` + `tb_config_filas`) + migração slug `tv_grupo` + `mutagen` duração real.
...
| Filas | `filas_modo_tv` (1), `filas_senha_prefixo` (A), `filas_guiche_padrao` (01), tema `filas_*` + multi-filas sem seed (`tv_grupo` slug + voz `voz_*` + textos `tv_*`, `tb_fila_etapa`/`tb_fila_nomes`/`tb_midia` por fila+global/`tb_tv_estado`/`tb_fila_acesso`, `/tv/{id}` vs `/tv?grupo=` vs `?etapa=`, `/midia_filas` `datahora_nomeFila`, claim+ducking 50%+hora, censura filtrada, notícias 200) | — | multi-filas por local + TV isolada/compartilhada/por sala + mídia + lista única — ver [Módulo Filas](modulos/filas.md) |
