...
- [x] Usuário inicial `master`/`master` com **troca de senha obrigatória no 1º logon** (seed em `../../mod_gest_cad_usuario/bd_manipulador.py`, flag `forcar_troca`)
...
- **`bloco_aparencia`**: cupê "Aparência" padronizado, agora **com botão "Salvar" próprio** e
  ...
  `edit_pdf`, `solicita_impressao` e `usuarios`, gravando via `salvar_tema` com auditoria.
- **`campo_modulo` — REMOVIDO (06/09)**: o cupê **"Edição do módulo"** (nome de exibição, ícone e
  status ativo em `tb_modulos`) foi retirado de todos os módulos e a função saiu de
  `tema_modulo.py` — essa edição é **exclusiva do painel central `/configuracoes`** (aba Módulo,
  admin geral).
