...
# Reatribui o CrudBase do módulo para apontar ao banco temporário (o `_crud`
# é criado no import com o caminho original).
bd._crud = CrudBase(bd.DB_BLOG_PATH, "blog")
...
def teste_config_local():
    ...
    # 30. postagem fixada do modo única (default = mais recente)
    assert bd.obter_postagem_unica_id() is None
    ok("postagem única default = mais recente (None)")
    # 31. fixar/limpar postagem única
    assert bd.definir_postagem_unica_id(3) is True
    assert bd.obter_postagem_unica_id() == 3
    ok("definir/obter_postagem_unica_id fixa o id")
    bd.definir_postagem_unica_id(None)
    assert bd.obter_postagem_unica_id() is None
    ok("definir_postagem_unica_id(None) limpa a fixação")
    # 32. seed idempotente da nova chave local
    chaves = [c for c, _ in bd.listar_config_local()]
    assert "blog_postagem_unica_id" in chaves
    ok("blog_postagem_unica_id semeada na config local")


def teste_carrossel():
    print("\n=== Teste: modo de exibição carrossel ===")
    # 32. modo carrossel aceito por obter_modo_exibicao
    bd.set_config_local("blog_modo_exibicao", "carrossel")
    assert bd.obter_modo_exibicao() == "carrossel"
    ok("obter_modo_exibicao aceita 'carrossel'")
    bd.set_config_local("blog_modo_exibicao", "historico")
    assert bd.obter_modo_exibicao() == "historico"
    ok("obter_modo_exibicao volta a 'historico'")
    # 33. tempo padrão do carrossel = 10
    assert bd.obter_carrossel_tempo() == 10
    ok("obter_carrossel_tempo padrão = 10")
    # 34. definir tempo
    assert bd.definir_carrossel_tempo(5) is True
    assert bd.obter_carrossel_tempo() == 5
    bd.definir_carrossel_tempo(10)
    ok("definir/obter_carrossel_tempo")
    # 35. definir/obter ids do carrossel
    assert bd.definir_carrossel_postagens_ids([1, 3, 2]) is True
    assert bd.obter_carrossel_postagens_ids() == [1, 3, 2]
    ok("definir/obter_carrossel_postagens_ids")
    # 36. listar por ids na ordem selecionada
    if _admin_disponivel():
        p1 = bd.criar_postagem("Carrossel A", "x", AUTOR_ADMIN)
        p2 = bd.criar_postagem("Carrossel B", "x", AUTOR_ADMIN)
        p3 = bd.criar_postagem("Carrossel C", "x", AUTOR_ADMIN)
        bd.definir_carrossel_postagens_ids([p2, p1, p3])
        ordenados = [p[0] for p in bd.listar_postagens_por_ids([p2, p1, p3])]
        assert ordenados == [p2, p1, p3], f"ordem inesperada {ordenados}"
        ok("listar_postagens_por_ids respeita a ordem selecionada")
        # ids inexistentes ignorados
        sem = bd.listar_postagens_por_ids([999999, p1])
        assert [p[0] for p in sem] == [p1]
        ok("listar_postagens_por_ids ignora ids inexistentes")
        # lista vazia
        assert bd.listar_postagens_por_ids([]) == []
        ok("listar_postagens_por_ids([]) = []")
    # 37. seeds de config do carrossel
    chaves = [c for c, _ in bd.listar_config_local()]
    assert "blog_carrossel_tempo" in chaves
    assert "blog_carrossel_postagens_ids" in chaves
    ok("chaves blog_carrossel_* semeadas na config local")
