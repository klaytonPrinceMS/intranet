"""Audit module screen — read-only viewer of the per-module audit trail.

Tela do módulo Auditoria — visualização SOMENTE-LEITURA da trilha de
auditoria LGPD gravada pelos demais módulos via `audit_log`. A trilha vive no
banco exclusivo `db_mod_auditoria.db`, com UMA TABELA POR MÓDULO
(`tb_auditoria_<modulo>`) — a navegação entre tabelas é montada
dinamicamente a partir do banco. Recursos: filtros (usuário/módulo/ação/
hora/intervalo de datas), paginação server-side, exportação CSV, categorias
de ação com cores e escolha do auditor sobre quais campos exibir e em que
ordem (persistida por usuário). Acesso exclusivo do `administrador_geral`.
"""
import sys, os
...
def mostrar_tela(usuario_logado: str, perfil: str):
    """Renders the read-only audit viewer (administrador_geral only).

    Monta a tela de auditoria: filtros (usuário, tabela de módulo dinâmica,
    ação com categorias coloridas, hora e intervalo de datas), painel de
    campos/ordem por auditor (persistido em `auditoria_campos:<usuario>`),
    tabela com paginação server-side, exportação CSV, aba Administração
    (limite de página, retenção LGPD, cabeçalho e aparência) e aba
    Observabilidade (dashboards Grafana) quando a stack OTel está no ar.
    ...
    tabs_el = abas("Logs", "history", observabilidade=obs_ligado)
        ...
        with ui.tab_panel("principal"):
            with ui.row().classes("w-full gap-3 flex-wrap mb-3 items-end"):
                filtro_usuario = ui.input("Filtrar usuário").props("outlined dense").classes("w-40") \
                    .props('data-testid=auditoria-busca')
                ...
                botao("Buscar", icone="search",
                      on_click=lambda: _atualizar_tabela(reset=True),
                      variante="solido", chave_modulo="auditoria")
                botao("Limpar", icone="clear", on_click=_limpar_filtros,
                      variante="texto", chave_modulo="auditoria")
                botao("Exportar CSV", icone="download", on_click=_exportar_csv,
                      variante="contorno", chave_modulo="auditoria") \
                    .props('data-testid=auditoria-exportar')
