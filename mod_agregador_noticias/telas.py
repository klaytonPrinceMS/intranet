    ...
    def _noticia_card(n):
        ...
        with ui.card().classes("w-full overflow-hidden hover:shadow-lg transition-shadow cursor-pointer").style("break-inside: avoid;"):
            ...
            with ui.card_section().classes("gap-1 w-full"):
                ...
                # titulo como link externo
                # Usa ui.link com target _blank para abrir externo
                try:
                ...
                with ui.row().classes("w-full items-center justify-between mt-1"):
                    ui.label(fonte or "").classes("text-caption text-grey-5")
                    ui.label((data_pub or data_col or "")[:16]).classes("text-caption text-grey-5")
                ...
                def _abrir():
                    try:
                        ...
                        pass
            # clique no card
            # nicegui card não tem on_click nativo, adiciona via class cursor-pointer + JS handler wrapper
            # fallback: botão invisível
            with ui.element("div").classes("w-full"):
                ui.button("Abrir notícia", on_click=lambda u=href: ui.run_javascript(f"window.open('{u}', '_blank')")).props("flat dense no-caps").classes("text-caption text-primary w-full")
    ...
    with ui.column().classes("w-full p-6 gap-4"):
        ...
        with ui.row().classes("w-full items-center gap-3 flex-wrap bg-white rounded-lg shadow-sm px-3 py-2"):
            ...
            if _pode_ver(user_nome, perfil_global) and (perfil_global == "administrador_geral" or autenticacao.eh_admin_do_modulo(user_nome, "agregador_noticias")):
                def _coletar():
                    n = ag.coletar_todas(ator=user_nome)
