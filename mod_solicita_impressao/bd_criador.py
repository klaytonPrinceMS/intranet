"""LEGACY database creator — mod_solicita_impressao.
...
criado e gerenciado por db_manipulador.py via init_db().

Este db_criador.py aponta para o banco CENTRAL (db_mod_intranet.db)
...
def init_db():
    ...
    O módulo usa db_mod_solicita_impressao.db (ver db_manipulador.py).
    """
    conn = get_connection()
    cur = conn.cursor()
